...
export function PhMeterSimulation({
  onRecord,
}: {
  onRecord: (reading: number) => void;
}) {
  ...
  const liveData = [
    ...
    stage === 6 ? point.time <= stabilization : point.time === 0,
  );
  if (stage === 6 && stabilization > 0)
    liveData.push({ time: stabilization, ph: reading });

  function attemptStep(target: PhStage) {
    setWarning("");
    if (target !== stage) {
      if (target === 5 && stage < 3)
        setWarning("Please calibrate the pH meter before measuring.");
      else if (target === 7 && !stabilized)
        setWarning("Wait for the pH reading to stabilize before recording it.");
      else
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Interactive pH Meter
            </CardTitle>
            <CardDescription>
              Perform every preparation step in sequence, then record the
              stabilized digital reading.
            </CardDescription>
          </div>
          <Badge variant={stage === 7 ? "default" : "outline"}>
            {statusLabel}
          </Badge>
        </div>
      </CardHeader>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(16rem,0.8fr)_minmax(20rem,1.2fr)]">
          <div className="relative flex min-h-[22rem] items-center justify-center overflow-hidden rounded-xl border bg-sky-500/5 p-6">
            ...
            <div className="relative mt-8 h-40 w-44 rounded-b-[3rem] border-2 border-slate-400 bg-background/60">
              <motion.div
                animate={{ height: stage >= 5 ? "46%" : "12%" }}
                className="absolute bottom-0 left-2 right-2 rounded-b-[2.5rem] bg-cyan-400/35"
              />
              <motion.div
                animate={{ y: stage >= 5 ? 44 : -34 }}
                transition={{ duration: 0.7 }}
                className="absolute left-1/2 top-0 h-20 w-2 -translate-x-1/2 rounded-full bg-slate-300 shadow-sm"
              />
              <div className="absolute -bottom-5 left-1/2 h-2 w-48 -translate-x-1/2 rounded-full bg-slate-400/50" />
            </div>
            <div className="absolute bottom-4 text-xs text-muted-foreground">
              Electrode:{" "}
              {stage >= 5 ? "in sample" : stage >= 4 ? "dry" : "ready"}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border bg-muted/20 p-4">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">Digital pH Meter</p>
                ...
                <div
                  className={`flex h-9 w-9 items-center justify-center rounded-full ${stage > 1 ? "bg-emerald-500/15 text-emerald-600" : "bg-muted text-muted-foreground"}`}
                >
                  <Power className="h-4 w-4" />
                </div>
              </div>
              <motion.div
                animate={{ scale: stage === 6 ? [1, 1.015, 1] : 1 }}
                transition={{
                  repeat: stage === 6 ? Infinity : 0,
                  duration: 1.2,
                }}
                className="rounded-lg border bg-slate-950 p-4 text-center text-4xl font-semibold tracking-widest text-emerald-400 shadow-inner"
              >
                {stage >= 5 ? reading.toFixed(2) : "--.--"}
              </motion.div>
              {stage === 6 && (
                <div className="mt-3">
            ...
            <div className="rounded-xl border p-4">
              ...
              {stage === 1 && (
                <Button className="w-full" onClick={() => attemptStep(1)}>
                  Switch ON pH Meter <Power className="ml-2 h-4 w-4" />
                </Button>
              )}
              {stage === 2 && (
                <Button className="w-full" onClick={() => attemptStep(2)}>
                  Wash Electrode <Droplets className="ml-2 h-4 w-4" />
                </Button>
              )}
              {stage === 3 && (
                <div className="space-y-3">
                  <p className="text-xs text-muted-foreground">
                    Choose the standard buffer used for calibration.
                  </p>
                  <div className="grid grid-cols-3 gap-2">
                    {["4.00", "7.00", "9.20"].map((value) => (
                      <Button
                        key={value}
                        type="button"
                        variant={buffer === value ? "default" : "outline"}
                        size="sm"
                        onClick={() => setBuffer(value)}
                      >
                        {value}
                      </Button>
                    ))}
              ...
              {stage === 4 && (
                <Button className="w-full" onClick={() => attemptStep(4)}>
                  Dry Electrode <Sparkles className="ml-2 h-4 w-4" />
                </Button>
              )}
              {stage === 5 && (
                <Button className="w-full" onClick={() => attemptStep(5)}>
                  Insert Electrode into Sample{" "}
              ...
              {stage === 6 && (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    The reading is approaching a stable value. Keep the
                    electrode immersed and wait.
                  </p>
                  <Button
                    className="w-full"
                    disabled={!stabilized}
                  ...
                  >
                    Record Reading <CheckCircle2 className="ml-2 h-4 w-4" />
                  </Button>
                </div>
              )}
              {stage === 7 && (
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-center text-sm text-emerald-700 dark:text-emerald-300">
                  ...
                  Reading recorded: pH 6.32
                </div>
              )}
            </div>
          </div>
        </div>
        <DashboardChart title="Live pH Stabilization" description="The electrode reading trends toward a stable pH value in real time." data={liveData.length ? liveData : [{ time: 0, ph: 7.1 }]} dataKey="ph" xKey="time" variant="line" color="#3b82f6" />
      </CardContent>
    </Card>
  );
}
